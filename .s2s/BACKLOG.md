...
1. **Implement** a `RequestValidationError` handler (in `vektra_shared/http_errors.py`, alongside the existing one) that wraps validation errors in the REQ-010 envelope with a stable code (e.g. `ERR-VALIDATION-001`). Meets REQ-010 literally, but it is a **wire-shape change** with the same weight as DEBT-034: audit the consumers first (the widget and the **vektra-moodle plugin** parse error bodies; the BUG-026 tests assert the `detail`-list shape and would flip). Getting the field mapping right (Pydantic's `loc`/`msg`/`type` into `message`/`details`) is the real work.
