"""Shared fixtures for vektra-ingest unit tests."""
...
def _hermetic_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    overriding config defaults (e.g. VEKTRA_CHUNKING_STRATEGY=dual makes
    run_ingest pick the wrong chunker under test). CI has no .env and never
    sees the difference. Tests that need a specific value still set it
