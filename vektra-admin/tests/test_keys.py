...
def test_verify_key_wrong_plaintext():
    _, key_hash, _ = generate_key()
    assert verify_key("wrong_token", key_hash) is False


def test_verify_key_does_not_raise():
    # Should return False rather than raising on a malformed hash
    assert verify_key("token", "not_a_valid_hash") is False
...
def test_lru_cache_hit_on_second_verify():
