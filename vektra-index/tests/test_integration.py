...
async def test_namespace_isolation(provider, embedding_provider, session):
    ...
    emb_a = await embedding_provider.embed_documents(
    ...
    async with session.begin():
        for ns in [ns_a, ns_b]:
            await session.execute(
                text(
                    "INSERT INTO namespaces (id, display_name) VALUES (:ns, :dn) ON CONFLICT (id) DO NOTHING"
                ),
                {"ns": ns, "dn": ns},
            )
    ...
    async with session.begin():
        results_a = await provider.search(
            session, ns_a, QueryEmbedding(dense=query_emb), top_k=10
        )
        results_b = await provider.search(
            session, ns_b, QueryEmbedding(dense=query_emb), top_k=10
        )
    ...
    assert a_ids.isdisjoint(b_ids), (
        "Namespace isolation failed: chunk appeared in both namespaces"
    )
...
async def test_delete_removes_chunks(provider, embedding_provider, session):
    ...
    async with session.begin():
        ...
        await provider.store(
            ...
            [
                ChunkEmbedding(
                    chunk_id="", text="chunk to delete", dense=embs[0], metadata={}
                )
            ],
        )
    ...
    assert len(results) == 0, (
        "Soft-deleted document should not appear in search results"
    )
